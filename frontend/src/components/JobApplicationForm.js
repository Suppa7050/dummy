...
const JobApplicationForm = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(`http://localhost:8001/api/jobs/${jobId}/questions/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8001/api/jobs/${jobId}/apply/`, applicationData);
